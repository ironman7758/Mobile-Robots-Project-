...
dispatchErrorNotify(PhidgetHandle phid, int throwError, Phidget_LogLevel lvl, const char *fmt, ...) {
	...
	if (throwError) {
		...
		if (_ch_) {
			Phidget_OnErrorCallback handler = _ch_->Error;
			void *ctx = (void *)_ch_->ErrorCtx;
			if (handler) {
				mos_vsnprintf(_errbuf_, sizeof(_errbuf_), fmt, va);
				handler((PhidgetHandle)_ch_, ctx, EEPHIDGET_DISPATCH, _errbuf_);
			}
		}
	}
...
dispatchEntry(PhidgetHandle phid, DispatchEntryHandle de) {
	...
	if (channel) {
		...
		case DE_CHANNEL_ATTACH: {
			...
			Phidget_OnAttachCallback ahandler = channel->Attach;
			void *actx = (void *)channel->AttachCtx;
			if (ahandler)
				ahandler((PhidgetHandle)channel, actx);
			...
			if (!isNetworkPhidget(channel)) {
				...
				if (res != EPHIDGET_OK) {
					...
					Phidget_OnDetachCallback dhandler = channel->Detach;
					void *dctx = (void *)channel->DetachCtx;
					if (dhandler) {
						PhidgetSetFlags(channel, PHIDGET_DETACHING_FLAG);
						PhidgetCLRFlags(channel, PHIDGET_ATTACHED_FLAG);
						dhandler((PhidgetHandle)channel, dctx);
		...
		case DE_CHANNEL_DETACH:
			PhidgetCLRFlags(channel, PHIDGET_ATTACHED_FLAG);
			Phidget_OnDetachCallback handler = channel->Detach;
			void *dctx = (void *)channel->DetachCtx;
			if (handler)
				handler((PhidgetHandle)channel, dctx);
	...
	if (manager) {
		switch (de->type) {
		case DE_MGR_ATTACHCH: {
			PhidgetManager_OnAttachCallback handler = manager->onAttach;
			void *ctx = (void *)manager->onAttachCtx;
			if (PhidgetCKFlags(manager, PHIDGET_OPEN_FLAG) != 0 && handler)
				handler(manager, ctx, de->de_phid);
		} break;
		case DE_MGR_DETACHCH: {
			PhidgetManager_OnDetachCallback handler = manager->onDetach;
			void *ctx = (void *)manager->onDetachCtx;
			if (PhidgetCKFlags(manager, PHIDGET_OPEN_FLAG) != 0 && handler)
				handler(manager, ctx, de->de_phid);
		} break;
		case DE_MGR_ATTACH:
