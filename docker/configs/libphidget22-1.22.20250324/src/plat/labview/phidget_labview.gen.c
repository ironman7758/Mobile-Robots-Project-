...
#ifdef COMPILE_PHIDGETS_LABVIEW
void CCONV PhidgetMotorPositionController_OnExpectedPositionChange_LaviewHandler(PhidgetMotorPositionControllerHandle ch,void *ctx, double expectedPosition) {
	LVUserEventRef ev = *(LVUserEventRef *)ctx;
	MgErr ret = 0;
	lvPhidgetMotorPositionControllerExpectedPositionChangeArgs *lvdata;
	lvdata = (lvPhidgetMotorPositionControllerExpectedPositionChangeArgs *)DSNewPtr(sizeof(lvPhidgetMotorPositionControllerExpectedPositionChangeArgs));
	lvdata->ch = (intptr_t)ch;
	lvdata->expectedPosition = expectedPosition;
	ret = PostLVUserEvent(ev, lvdata);
	DSDisposePtr(lvdata);
}

API_PRETURN_HDR PhidgetMotorPositionController_setOnExpectedPositionChangeLabviewHandler(PhidgetMotorPositionControllerHandle ch, LVUserEventRef *lvEventRef) {
	TESTPTR(ch);
	if(lvEventRef && *lvEventRef)
		PhidgetMotorPositionController_setOnExpectedPositionChangeHandler(ch,
		  PhidgetMotorPositionController_OnExpectedPositionChange_LaviewHandler, lvEventRef);
	else
		PhidgetMotorPositionController_setOnExpectedPositionChangeHandler(ch, NULL, NULL);
