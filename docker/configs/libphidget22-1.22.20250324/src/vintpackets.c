...
VINTPacketStatusCodeDeviceCode_to_PhidgetReturnCode(VINTPacketStatusCode code, const PhidgetUniqueDeviceDef *udd) {
	...
	case PHIDUID_DCC1130:
		...
		case VINTPacketStatusCode_DCC11X0_BADCONN:
	...
	case PHIDUID_DCC1030:
		...
		case VINTPacketStatusCode_DCC10X0_INIT:
			return (EPHIDGET_BADCURRENT);
		case VINTPacketStatusCode_DCC10X0_BADCONN:
			return (EPHIDGET_BADCONNECTION);
		}
		break;
#endif
...
sendFirmware(mosiop_t iop, PhidgetChannelHandle ch, const uint8_t *data, size_t dataLen) {
	...
	do {
		...
		case PHIDCHUID_STM32F0_FIRMWARE_UPGRADE_400:
			...
			memcpy(buffer + 4, data + offset, 32);

			res = sendVINTDataPacket(iop, ch, VINT_BOOTLOADER_PACKET_TYPE_FIRMWAREUPGRADEDATA_BIGOFFSET, buffer, 36);
			break;
		...
		if (res != EPHIDGET_OK) {
			...
			logdebug("Sent some firmware data: %d bytes at 0x%08zx", 32, offset);
...
static PhidgetReturnCode
sendFIRMWARE_UPGRADE_STM32F0v2(PhidgetChannelHandle ch, BridgePacket *bp) {
	PhidgetReturnCode res;
	size_t base64Len;
	void *base64;
	int off;
...
static PhidgetReturnCode
recvFIRMWARE_UPGRADE_STM32F0v2(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	Phidget_DeviceHardwareID deviceID;
	int version;
	int pkt;
...
#if PHIDUID_1024_1_VINT_OBSOLETE_SUPPORTED
static PhidgetReturnCode
send1024_1_VINT_OBSOLETE(PhidgetChannelHandle ch, BridgePacket *bp) {
	MOS_PANIC("Obsolete Firmware");
}

static PhidgetReturnCode
recv1024_1_VINT_OBSOLETE(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {

	logwarn("Ignoring incoming packet from obsolete firmware");
	return (EPHIDGET_OK);
}
#endif /* PHIDUID_1024_1_VINT_OBSOLETE_SUPPORTED */

#if PHIDUID_1024_1_VINT_SUPPORTED
static PhidgetReturnCode
send1024_1_VINT(PhidgetChannelHandle ch, BridgePacket *bp) {
	...
	case PHIDCHUID_1024_RFID_320_VINT:
		...
		case BP_WRITEWITHCHIPSET:
			if (ch->UCD->uid == PHIDCHUID_1024_RFID_300_VINT || ch->UCD->uid == PHIDCHUID_1024_RFID_310_VINT) {
				TESTRANGE_IOP(bp->iop, "%d", getBridgePacketInt32(bp, 3), CHIPSET_T5577, CHIPSET_T5577);
			} else {
				TESTRANGE_IOP(bp->iop, "%d", getBridgePacketInt32(bp, 3), CHIPSET_T5577, CHIPSET_EM4305);
			}
			// Fall through
		case BP_WRITE:
			protocol = getBridgePacketInt32(bp, 1);
			if (ch->UCD->uid == PHIDCHUID_1024_RFID_300_VINT)
				TESTRANGE_IOP(bp->iop, "%d", protocol, PROTOCOL_EM4100, PROTOCOL_PHIDGETS);
			else
				TESTRANGE_IOP(bp->iop, "%d", protocol, PROTOCOL_EM4100, PROTOCOL_HID_H10301);
			buffer[0] = getBridgePacketInt32(bp, 1);
			buffer[1] = getBridgePacketInt32(bp, 2) != 0;

			tagString = getBridgePacketString(bp, 0);
			if ((protocol == PROTOCOL_EM4100 || protocol == PROTOCOL_HID_GENERIC) && tagString[1] == 'x')
				tagString += 2;
			...
			if (protocol == PROTOCOL_EM4100 || protocol == PROTOCOL_HID_GENERIC) {
				for (i = 0; (size_t)i < stringLength; i++) {
			...
			} else { /* PHIDUID_1024_1_320_VINT and later */
				if (bp->vpkt == BP_WRITEWITHCHIPSET) {
					buffer[len] = getBridgePacketInt32(bp, 3); // reserve space in case we decide to indicate which chipset to write
					len++;
					ret = sendVINTDataPacket(bp->iop, ch, VINT_PACKET_TYPE_RFID_TAG_WRITECHIPSET, buffer, len);
				} else {
					ret = sendVINTDataPacket(bp->iop, ch, VINT_PACKET_TYPE_RFID_TAG_WRITE, buffer, len);
				}
			}
			...
			memcpy(sentTag, (char *)&buffer[2], stringLength);
			sentTag[stringLength] = '\0';

			PhidgetRFIDSupport_setExpectedTag(protocol, sentTag, expectedTag);
			if (ch->UCD->uid == PHIDCHUID_1024_RFID_300_VINT || ch->UCD->uid == PHIDCHUID_1024_RFID_310_VINT)
				return RFIDSupport_waitForTag(bp->iop, ch, (const char *)expectedTag, 700, NULL);
			else /* PHIDCHUID_1024_RFID_320_VINT and later */
				return RFIDSupport_waitForTag(bp->iop, ch, (const char *)expectedTag, 1000, NULL);
		default:
			MOS_PANIC("Unexpected packet type");
		}
...
static PhidgetReturnCode
recv1024_1_VINT(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	int pkt;
...
static PhidgetReturnCode
send1024_1_310_VINT(PhidgetChannelHandle ch, BridgePacket *bp) {
	return send1024_1_VINT(ch, bp);
}

static PhidgetReturnCode
recv1024_1_310_VINT(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	return recv1024_1_VINT(ch, buf, len);
}

static PhidgetReturnCode
send1024_1_320_VINT(PhidgetChannelHandle ch, BridgePacket *bp) {
	return send1024_1_VINT(ch, bp);
}

static PhidgetReturnCode
recv1024_1_320_VINT(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	return recv1024_1_VINT(ch, buf, len);
}
#endif /* PHIDUID_1024_1_VINT_SUPPORTED */
...
static PhidgetReturnCode
sendADP0001_VINT(PhidgetChannelHandle ch, BridgePacket *bp) {
	...
	case PHIDCHUID_ADP0001_DATAADAPTER_100_VINT:
		...
		case BP_SETIOVOLTAGE:
			...
			ret = sendVINTDataPacket(bp->iop, ch, VINT_PACKET_TYPE_DATAADAPTER_IOVOLTAGE, buf, 1);
	...
	case PHIDCHUID_ADP0001_DIGITALOUTPUT_100_VINT:
		switch (bp->vpkt) {
		case BP_SETIOVOLTAGE:
			buf[0] = (uint8_t)getBridgePacketInt32(bp, 0);
			ret = sendVINTDataPacket(bp->iop, ch, VINT_PACKET_TYPE_DATAADAPTER_IOVOLTAGE, buf, 1);
	...
	case PHIDCHUID_ADP0001_DIGITALINPUT_100_VINT:
		switch (bp->vpkt) {
		case BP_SETIOVOLTAGE:
			buf[0] = (uint8_t)getBridgePacketInt32(bp, 0);
			ret = sendVINTDataPacket(bp->iop, ch, VINT_PACKET_TYPE_DATAADAPTER_IOVOLTAGE, buf, 1);
		...
		case BP_SETINPUTMODE:
			...
			ret = sendVINTDataPacket(bp->iop, ch, VINT_PACKET_TYPE_DIGITALINPUT_SETINPUTMODE, buf, 1);
			return (ret);
		default:
			MOS_PANIC("Unexpected packet type");
		}
...
static PhidgetReturnCode
recvADP0001_VINT(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	int state;
	int pkt;
	assert(buf);
...
static PhidgetReturnCode
sendADP0002_VINT(PhidgetChannelHandle ch, BridgePacket *bp) {
	PhidgetVINTDeviceHandle phid = (PhidgetVINTDeviceHandle)ch->parent;
	PhidgetReturnCode ret;
	double dutyCycle;
	uint8_t buf[1];
	...
	case PHIDCHUID_ADP0002_DIGITALOUTPUT_100_VINT:
		switch (bp->vpkt) {
		case BP_SETIOVOLTAGE:
			buf[0] = (uint8_t)getBridgePacketInt32(bp, 0);
			ret = sendVINTDataPacket(bp->iop, ch, VINT_PACKET_TYPE_DATAADAPTER_IOVOLTAGE, buf, 1);
	...
	case PHIDCHUID_ADP0002_DIGITALINPUT_100_VINT:
		switch (bp->vpkt) {
		case BP_SETIOVOLTAGE:
			buf[0] = (uint8_t)getBridgePacketInt32(bp, 0);
			ret = sendVINTDataPacket(bp->iop, ch, VINT_PACKET_TYPE_DATAADAPTER_IOVOLTAGE, buf, 1);
...
static PhidgetReturnCode
recvADP0002_VINT(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	int state;
	int pkt;
	assert(buf);
...
static PhidgetReturnCode
sendLED0100_VINT(PhidgetChannelHandle ch, BridgePacket *bp) {
	assert(ch);
	assert(bp);
	...
	case PHIDCHUID_LED0100_LEDARRAY_100_VINT:
		switch (bp->vpkt) {
		default:
			return (PhidgetLEDArraySupport_bridgeInput(ch, bp));
		}
	default:
		MOS_PANIC("Invalid Channel UID");
	}
...
static PhidgetReturnCode
recvLED0100_VINT(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	assert(buf);
	...
	case PHIDCHUID_LED0100_LEDARRAY_100_VINT:
		switch (buf[0]) {
		default:
			return (PhidgetLEDArraySupport_dataInput(ch, buf, len));
		}
	default:
		MOS_PANIC("Invalid Channel UID");
	}
}
#endif /* PHIDUID_LED0100_VINT_SUPPORTED */
...
#if PHIDUID_TMP1200_2_OBSOLETE_SUPPORTED
static PhidgetReturnCode
sendTMP1200_2_OBSOLETE(PhidgetChannelHandle ch, BridgePacket *bp) {
	MOS_PANIC("Obsolete Firmware");
}

static PhidgetReturnCode
recvTMP1200_2_OBSOLETE(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {

	logwarn("Ignoring incoming packet from obsolete firmware");
	return (EPHIDGET_OK);
}
#endif /* PHIDUID_TMP1200_2_OBSOLETE_SUPPORTED */
...
static PhidgetReturnCode
sendTMP1202_0(PhidgetChannelHandle ch, BridgePacket *bp) {
	PhidgetVINTDeviceHandle phid = (PhidgetVINTDeviceHandle)ch->parent;
	PhidgetReturnCode ret;
	uint8_t buf[4];
	...
	switch (ch->UCD->uid) {
	case PHIDCHUID_TMP1202_RESISTANCEINPUT_100:
		switch (bp->vpkt) {
		case BP_SETDATAINTERVAL:
			pack16(buf, HANDLE_DATAINTERVAL_PKT(bp, 1));
			return (sendVINTDataPacket(bp->iop, ch, VINT_PACKET_TYPE_SAMPLED_SETDATAINTERVAL, buf, 2));
	...
	case PHIDCHUID_TMP1202_TEMPERATURESENSOR_RTD_100:
		switch (bp->vpkt) {
		case BP_SETDATAINTERVAL:
			pack16(buf, HANDLE_DATAINTERVAL_PKT(bp, 1));
			return (sendVINTDataPacket(bp->iop, ch, VINT_PACKET_TYPE_SAMPLED_SETDATAINTERVAL, buf, 2));
...
static PhidgetReturnCode
recvTMP1202_0(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	PhidgetReturnCode result;
	double temperature;
	double resistance;
	int interval;
	...
	case PHIDCHUID_TMP1202_RESISTANCEINPUT_100:
		...
		case VINT_PACKET_TYPE_RESISTANCEINPUT_RESISTANCECHANGE:
			...
			if (len > 4) {
				if (isnan(resistance))
					PhidgetChannel_sendErrorEventThrottled(ch, EEPHIDGET_BADCONNECTION, "Bad Connection: Input is likely disconnected, or resistance is too high to be measured");
			...
			return (bridgeSendToChannel(ch, BP_RESISTANCECHANGE, 1, "%g", resistance));
		case VINT_PACKET_TYPE_RESISTANCEINPUT_MAXRES_CAHNGE:
			return bridgeSendToChannel(ch, BP_MAXRESISTANCECHANGE, 1, "%g", unpackfloat(buf));
		case VINT_PACKET_TYPE_DATAINTERVAL_RESP:
			interval = unpack16(buf);
			result = bridgeSendToChannel(ch, BP_DATAINTERVALCHANGE, 1, "%u", interval);
			return result;
		default:
			MOS_PANIC("Unexpected packet type");
		}
...
#if PHIDUID_TMP0102_VINT_SUPPORTED
static PhidgetReturnCode
sendTMP0102_VINT(PhidgetChannelHandle ch, BridgePacket *bp) {
	uint8_t buf[4];
	...
	switch (ch->UCD->uid) {
	case PHIDCHUID_TMP0102_VOLTAGEINPUT_100_VINT:
		switch (bp->vpkt) {
		case BP_SETDATAINTERVAL:
			pack16(buf, HANDLE_DATAINTERVAL_PKT(bp, 1));
			return (sendVINTDataPacket(bp->iop, ch, VINT_PACKET_TYPE_SAMPLED_SETDATAINTERVAL, buf, 2));
	...
	case PHIDCHUID_TMP0102_TEMPERATURESENSOR_IC_100_VINT:
		switch (bp->vpkt) {
		case BP_SETDATAINTERVAL:
			pack16(buf, HANDLE_DATAINTERVAL_PKT(bp, 1));
			return (sendVINTDataPacket(bp->iop, ch, VINT_PACKET_TYPE_SAMPLED_SETDATAINTERVAL, buf, 2));
	...
	case PHIDCHUID_TMP0102_TEMPERATURESENSOR_THERMOCOUPLE_100_VINT:
		switch (bp->vpkt) {
		case BP_SETDATAINTERVAL:
			pack16(buf, HANDLE_DATAINTERVAL_PKT(bp, 1));
			return (sendVINTDataPacket(bp->iop, ch, VINT_PACKET_TYPE_SAMPLED_SETDATAINTERVAL, buf, 2));
...
static PhidgetReturnCode
recvTMP0102_VINT(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	...
	case PHIDCHUID_TMP0102_VOLTAGEINPUT_100_VINT:
		...
		case VINT_PACKET_TYPE_VOLTAGEINPUT_VOLTAGECHANGE:
			voltage = unpackfloat(buf);
			if (!isnan(voltage))
				voltage = round_double(voltage, 9);

			if (len > 4) {
				switch (buf[4] & 0x07) {
				case 0x01:
					ret = PhidgetChannel_sendErrorEvent(ch, EEPHIDGET_OUTOFRANGELOW, "Voltage is too low to be accurately measured");
					break;
				case 0x02:
					ret = PhidgetChannel_sendErrorEvent(ch, EEPHIDGET_OUTOFRANGEHIGH, "Voltage is too high to be accurately measured. This may also indicate the thermocouple is disconnected.");
					break;
				case 0x03:
					ret = PhidgetChannel_sendErrorEvent(ch, EEPHIDGET_OUTOFRANGE, "Voltage is unknown");
					break;
				case 0x04:
				case 0x05:
				case 0x06:
				case 0x07:
					ret = PhidgetChannel_sendErrorEvent(ch, EEPHIDGET_OUTOFRANGE, "Voltage is unknown. Input is outside the acceptable common-mode range. This typically indicates grounding issues or other wiring problems.");
					break;
				}
			}
			if (ret != EPHIDGET_OK)
	...
	case PHIDCHUID_TMP0102_TEMPERATURESENSOR_THERMOCOUPLE_100_VINT:
		...
		case VINT_PACKET_TYPE_TEMPERATURESENSOR_TEMPERATURECHANGE:
			...
			if (!isnan(temperature))
				temperature = round_double(temperature, 6);
			...
			if (len > 4) {
				...
				case 0x01:
					...
					break;
				case 0x02:
					ret = PhidgetChannel_sendErrorEvent(ch, EEPHIDGET_OUTOFRANGEHIGH, "Temperature is too high to be accurately measured. This may also indicate the thermocouple is disconnected.");
					break;
				case 0x03:
				...
				case 0x06:
				case 0x07:
					ret = PhidgetChannel_sendErrorEvent(ch, EEPHIDGET_OUTOFRANGE, "Temperature is unknown. Input is outside the acceptable common-mode range. This typically indicates grounding issues or other wiring problems.");
					break;
				}
			}
			if (ret != EPHIDGET_OK)
...
#endif // PHIDUID_TMP0102_VINT_SUPPORTED
...
recvSTC1003_1(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	...
	case PHIDCHUID_STC1003_VOLTAGEINPUT_200:
		switch (pkt) {
		case VINT_PACKET_TYPE_VOLTAGEINPUT_VOLTAGECHANGE:
			voltage = round_double(unpackfloat(buf), 2);
			return (bridgeSendToChannel(ch, BP_VOLTAGECHANGE, 1, "%g", voltage));
		case VINT_PACKET_TYPE_VOLTAGEINPUT_SATURATION:
			return (PhidgetChannel_sendErrorEventThrottled(ch, EEPHIDGET_SATURATION, "Saturation Detected."));
		default:
...
recvSAF1000(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	...
	case PHIDCHUID_SAF1000_VOLTAGEINPUT_110:
		switch (pkt) {
		case VINT_PACKET_TYPE_VOLTAGEINPUT_VOLTAGECHANGE:
			voltage = round_double(unpacku32xS(buf, 24), 2);
			return (bridgeSendToChannel(ch, BP_VOLTAGECHANGE, 1, "%g", voltage));
		case VINT_PACKET_TYPE_VOLTAGEINPUT_SATURATION:
			return (PhidgetChannel_sendErrorEventThrottled(ch, EEPHIDGET_SATURATION, "Saturation Detected."));
		default:
...
#if PHIDUID_DCC1100_1_SUPPORTED
	case PHIDCHUID_DCC1100_1_MOTORPOSITIONCONTROLLER_200:
#endif
		switch (bp->vpkt) {
		case BP_SETDEADBAND:
			pack32(buf, getBridgePacketUInt32(bp, 0));
...
#if PHIDUID_DCC1100_1_SUPPORTED
	case PHIDCHUID_DCC1100_1_TEMPERATURESENSOR_IC_200:
#endif
		switch (bp->vpkt) {
		case BP_SETDATAINTERVAL:
			pack16(buf, HANDLE_DATAINTERVAL_PKT(bp, 1));
...
#if PHIDUID_DCC1100_1_SUPPORTED
	case PHIDCHUID_DCC1100_1_BLDCMOTOR_200:
#endif
		switch (bp->vpkt) {
		case BP_SETACCELERATION:
			pack32(buf, (uint32_t)((getBridgePacketDouble(bp, 0)) * 8192));
...
#if PHIDUID_DCC1100_1_SUPPORTED
static PhidgetReturnCode
sendDCC1100_1(PhidgetChannelHandle ch, BridgePacket *bp) {
...
sendDCC1000(PhidgetChannelHandle ch, BridgePacket *bp) {
	...
	case PHIDCHUID_DCC1000_VOLTAGERATIOINPUT_100:
		...
		case BP_SETDATAINTERVAL:
			...
			return (sendVINTDataPacket(bp->iop, ch, VINT_PACKET_TYPE_SAMPLED_SETDATAINTERVAL, buf, 2));
		case BP_SETCHANGETRIGGER:
			pack32(buf, (uint32_t)(getBridgePacketDouble(bp, 0) * ((1 << 16)-1)));
...
sendDCC1020(PhidgetChannelHandle ch, BridgePacket *bp) {
	...
	case PHIDCHUID_DCC1010_CURRENTINPUT_100:
		...
		case BP_SETDATAINTERVAL:
			...
			return (sendVINTDataPacket(bp->iop, ch, VINT_PACKET_TYPE_SAMPLED_SETDATAINTERVAL, buf, 2));
		case BP_SETCHANGETRIGGER:
			pack16(buf, (uint16_t)ceil((((4095 / 3.3) * 0.066)) * getBridgePacketDouble(bp, 0)));
			return (sendVINTDataPacket(bp->iop, ch, VINT_PACKET_TYPE_CURRENTINPUT_SETCURRENTCHANGETRIGGER, buf, 2));
		default:
			MOS_PANIC("Unexpected packet type");
		}
	...
	case PHIDCHUID_DCC1020_CURRENTINPUT_100:
		...
		case BP_SETDATAINTERVAL:
			...
			return (sendVINTDataPacket(bp->iop, ch, VINT_PACKET_TYPE_SAMPLED_SETDATAINTERVAL, buf, 2));
		case BP_SETCHANGETRIGGER:
			pack16(buf, (uint16_t)ceil((((4095 / 3.3) * 0.0132)) * getBridgePacketDouble(bp, 0)));
			return (sendVINTDataPacket(bp->iop, ch, VINT_PACKET_TYPE_CURRENTINPUT_SETCURRENTCHANGETRIGGER, buf, 2));
		default:
			MOS_PANIC("Unexpected packet type");
		}
	...
	case PHIDCHUID_DCC1030_DCMOTOR_100:
		...
		case BP_SETFAILSAFEBRAKINGDUTYCYCLE:
			packfloat(buf, (double)(getBridgePacketInt32(bp, 0)));
...
#if PHIDUID_DCC1100_1_SUPPORTED
static PhidgetReturnCode
recvDCC1100_1(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
...
#if PHIDUID_DAQ1400_130_SUPPORTED
...
#if PHIDUID_DAQ1400_130_SUPPORTED
	case PHIDCHUID_DAQ1400_CURRENTINPUT_130:
#endif
		switch (bp->vpkt) {
		case BP_SETDATAINTERVAL:
			pack16(buf, HANDLE_DATAINTERVAL_PKT(bp, 1));
...
#if PHIDUID_DAQ1400_130_SUPPORTED
static PhidgetReturnCode
sendDAQ1400_130(PhidgetChannelHandle ch, BridgePacket *bp) {
	return sendDAQ1400(ch, bp);
}

static PhidgetReturnCode
recvDAQ1400_130(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	return recvDAQ1400(ch, buf, len);
}
#endif
...
#if PHIDUID_DAQ1001_SUPPORTED

static PhidgetReturnCode
sendDAQ1001(PhidgetChannelHandle ch, BridgePacket *bp) {
	uint8_t buf[4];
	...
	switch (ch->UCD->uid) {
	case PHIDCHUID_DAQ1001_VOLTAGEINPUT_100:
		switch (bp->vpkt) {
		case BP_SETDATAINTERVAL:
			pack16(buf, HANDLE_DATAINTERVAL_PKT(bp, 1));
			return (sendVINTDataPacket(bp->iop, ch, VINT_PACKET_TYPE_SAMPLED_SETDATAINTERVAL, buf, 2));
	...
	case PHIDCHUID_DAQ1001_VOLTAGERATIOINPUT_100:
		switch (bp->vpkt) {
		case BP_SETDATAINTERVAL:
			pack16(buf, HANDLE_DATAINTERVAL_PKT(bp, 1));
			return (sendVINTDataPacket(bp->iop, ch, VINT_PACKET_TYPE_SAMPLED_SETDATAINTERVAL, buf, 2));
...
static PhidgetReturnCode
recvDAQ1001(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	const double voltageSaturationMarker = 1000.0;
	PhidgetReturnCode ret;
	double voltage;
	int interval;
	int pkt;
	...
	case PHIDCHUID_DAQ1001_VOLTAGEINPUT_100:
		switch (pkt) {
		case VINT_PACKET_TYPE_VOLTAGEINPUT_VOLTAGECHANGE:
			voltage = round_double(unpackfloat(buf), 5);
			if (voltage > voltageSaturationMarker) {
				PhidgetChannel_sendErrorEvent(ch, EEPHIDGET_OUTOFRANGEHIGH, "Voltage is too high to be accurately measured");
				voltage -= voltageSaturationMarker;
				voltage = round_double(voltage, 3);
			}
			return (bridgeSendToChannel(ch, BP_VOLTAGECHANGE, 1, "%g", voltage));
		case VINT_PACKET_TYPE_DATAINTERVAL_RESP:
			interval = unpack16(buf);
