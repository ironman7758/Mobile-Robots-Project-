...
PhidgetRCServo_bridgeInput(PhidgetChannelHandle phid, BridgePacket *bp) {
	...
	case BP_POSITIONCHANGE:
		...
		FIRECH(ch, PhidgetRCServo, PositionChange, POSITION_USER(ch->position));
		res = EPHIDGET_OK;
		break;
	case BP_VELOCITYCHANGE:
		ch->velocity = getBridgePacketDouble(bp, 0);
		FIRECH(ch, PhidgetRCServo, VelocityChange, VELOCITY_USER(ch->velocity));
		res = EPHIDGET_OK;
		break;
	case BP_TARGETPOSITIONREACHED:
		ch->isMoving = PFALSE;
		ch->position = getBridgePacketDouble(bp, 0);
		FIRECH(ch, PhidgetRCServo, TargetPositionReached, POSITION_USER(ch->position));
		res = EPHIDGET_OK;
		break;
	case BP_SETENGAGED:
