...
struct _PhidgetAccelerometer {
	...
	double timestamp;
	volatile PhidgetAccelerometer_OnAccelerationChangeCallback AccelerationChange;
	volatile void *AccelerationChangeCtx;
};
...
_bridgeInput(PhidgetChannelHandle phid, BridgePacket *bp) {
	...
	case BP_ACCELERATIONCHANGE:
		...
		FIRECH(ch, PhidgetAccelerometer, AccelerationChange, ch->acceleration, ch->timestamp);
		break;
	default:
...
PhidgetAccelerometer_setOnAccelerationChangeHandler(PhidgetAccelerometerHandle ch,
  PhidgetAccelerometer_OnAccelerationChangeCallback fptr, void *ctx) {
	...
	MEMORY_BARRIER();
	if (fptr == NULL) {
		ch->AccelerationChange = NULL;
		MEMORY_BARRIER();
		ch->AccelerationChangeCtx = NULL;
	} else {
