...
struct _PhidgetSpatial {
	...
	volatile PhidgetSpatial_OnAlgorithmDataCallback AlgorithmData;
	volatile void *AlgorithmDataCtx;
	volatile PhidgetSpatial_OnSpatialDataCallback SpatialData;
	volatile void *SpatialDataCtx;
};
...
_bridgeInput(PhidgetChannelHandle phid, BridgePacket *bp) {
	...
	case BP_SETSPATIALPRECISION:
		...
		break;
	case BP_SPATIALDATA:
		FIRECH(ch, PhidgetSpatial, SpatialData, getBridgePacketDoubleArray(bp, 0),
		  getBridgePacketDoubleArray(bp, 1), getBridgePacketDoubleArray(bp, 2), getBridgePacketDouble(bp, 3));
		break;
	default:
...
PhidgetSpatial_setOnAlgorithmDataHandler(PhidgetSpatialHandle ch,
  PhidgetSpatial_OnAlgorithmDataCallback fptr, void *ctx) {
	...
	MEMORY_BARRIER();
	if (fptr == NULL) {
		ch->AlgorithmData = NULL;
		MEMORY_BARRIER();
		ch->AlgorithmDataCtx = NULL;
	} else {
