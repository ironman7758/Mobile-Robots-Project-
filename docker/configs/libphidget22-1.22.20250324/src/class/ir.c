...
PhidgetIR_bridgeInput(PhidgetChannelHandle phid, BridgePacket *bp) {
	...
	case BP_CODE:
		...
		FIRECH(ch, PhidgetIR, Code, ch->lastCodeStr, ch->lastCodeInfo.bitCount, lastRepeat);
		res = EPHIDGET_OK;
		break;

	case BP_REPEAT:
		FIRECH(ch, PhidgetIR, Code, ch->lastCodeStr, ch->lastCodeInfo.bitCount, PTRUE);
		res = EPHIDGET_OK;
		break;
	...
	case BP_LEARN:
		...
		FIRECH(ch, PhidgetIR, Learn, codeStr, &codeInfo);
	...
	case BP_RAWDATA:
		FIRECH(ch, PhidgetIR, RawData, getBridgePacketUInt32Array(bp, 0), getBridgePacketArrayCnt(bp, 0));
		res = EPHIDGET_OK;
		break;
