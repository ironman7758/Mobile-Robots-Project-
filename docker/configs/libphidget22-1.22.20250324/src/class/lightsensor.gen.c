...
struct _PhidgetLightSensor {
	...
	volatile PhidgetLightSensor_OnIlluminanceChangeCallback IlluminanceChange;
	volatile void *IlluminanceChangeCtx;
};
...
_bridgeInput(PhidgetChannelHandle phid, BridgePacket *bp) {
	...
	case BP_ILLUMINANCECHANGE:
		...
		FIRECH(ch, PhidgetLightSensor, IlluminanceChange, ch->illuminance);
		break;
	default:
...
PhidgetLightSensor_setOnIlluminanceChangeHandler(PhidgetLightSensorHandle ch,
  PhidgetLightSensor_OnIlluminanceChangeCallback fptr, void *ctx) {
	...
	MEMORY_BARRIER();
	if (fptr == NULL) {
		ch->IlluminanceChange = NULL;
		MEMORY_BARRIER();
		ch->IlluminanceChangeCtx = NULL;
	} else {
