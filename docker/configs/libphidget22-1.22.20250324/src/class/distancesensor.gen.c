...
struct _PhidgetDistanceSensor {
	...
	int sonarQuietMode;
	volatile PhidgetDistanceSensor_OnDistanceChangeCallback DistanceChange;
	volatile void *DistanceChangeCtx;
	volatile PhidgetDistanceSensor_OnSonarReflectionsUpdateCallback SonarReflectionsUpdate;
	volatile void *SonarReflectionsUpdateCtx;
};
...
_bridgeInput(PhidgetChannelHandle phid, BridgePacket *bp) {
	...
	case BP_DISTANCECHANGE:
		...
		FIRECH(ch, PhidgetDistanceSensor, DistanceChange, ch->distance);
		break;
	default:
...
PhidgetDistanceSensor_setOnDistanceChangeHandler(PhidgetDistanceSensorHandle ch,
  PhidgetDistanceSensor_OnDistanceChangeCallback fptr, void *ctx) {
	...
	MEMORY_BARRIER();
	if (fptr == NULL) {
		ch->DistanceChange = NULL;
		MEMORY_BARRIER();
		ch->DistanceChangeCtx = NULL;
	} else {
...
PhidgetDistanceSensor_setOnSonarReflectionsUpdateHandler(PhidgetDistanceSensorHandle ch,
  PhidgetDistanceSensor_OnSonarReflectionsUpdateCallback fptr, void *ctx) {
	...
	MEMORY_BARRIER();
	if (fptr == NULL) {
		ch->SonarReflectionsUpdate = NULL;
		MEMORY_BARRIER();
		ch->SonarReflectionsUpdateCtx = NULL;
	} else {
