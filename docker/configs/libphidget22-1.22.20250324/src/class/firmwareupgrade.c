...
PhidgetFirmwareUpgrade_setStatus(PhidgetChannelHandle phid, BridgePacket *bp) {
	...
	if (isVintChannel(phid)) {
		...
		while (((int)pdd->type) != END_OF_LIST) {
			if (pdd->type == PHIDTYPE_VINT && ch->actualDeviceVINTID == pdd->vintID && ch->actualDeviceID == pdd->id && ((ch->actualDeviceVersion >= pdd->versionLow && ch->actualDeviceVersion < pdd->versionHigh) || ch->actualDeviceVersion == 0)) {
...
PhidgetFirmwareUpgrade_bridgeInput(PhidgetChannelHandle phid, BridgePacket *bp) {
	...
	case BP_DEVICEINFO:
		...
		pdd = Phidget_Unique_Device_Def;
		while (((int)pdd->type) != END_OF_LIST) {
			if (pdd->type == PHIDTYPE_VINT && (unsigned short)vintId == pdd->vintID && ((ch->actualDeviceVersion >= pdd->versionLow && ch->actualDeviceVersion < pdd->versionHigh) || ch->actualDeviceVersion == 0)) {
				ch->actualDeviceSKU = pdd->SKU;
				ch->actualDeviceName = pdd->name;
