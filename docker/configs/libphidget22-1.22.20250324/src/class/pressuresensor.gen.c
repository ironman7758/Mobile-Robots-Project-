...
struct _PhidgetPressureSensor {
	...
	volatile PhidgetPressureSensor_OnPressureChangeCallback PressureChange;
	volatile void *PressureChangeCtx;
};
...
_bridgeInput(PhidgetChannelHandle phid, BridgePacket *bp) {
	...
	case BP_PRESSURECHANGE:
		...
		FIRECH(ch, PhidgetPressureSensor, PressureChange, ch->pressure);
		break;
	default:
...
PhidgetPressureSensor_setOnPressureChangeHandler(PhidgetPressureSensorHandle ch,
  PhidgetPressureSensor_OnPressureChangeCallback fptr, void *ctx) {
	...
	MEMORY_BARRIER();
	if (fptr == NULL) {
		ch->PressureChange = NULL;
		MEMORY_BARRIER();
		ch->PressureChangeCtx = NULL;
	} else {
