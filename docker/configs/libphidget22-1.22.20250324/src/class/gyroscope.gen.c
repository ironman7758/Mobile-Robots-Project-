...
struct _PhidgetGyroscope {
	...
	double timestamp;
	volatile PhidgetGyroscope_OnAngularRateUpdateCallback AngularRateUpdate;
	volatile void *AngularRateUpdateCtx;
};
...
_bridgeInput(PhidgetChannelHandle phid, BridgePacket *bp) {
	...
	case BP_ANGULARRATEUPDATE:
		...
		FIRECH(ch, PhidgetGyroscope, AngularRateUpdate, ch->angularRate, ch->timestamp);
		break;
	default:
...
PhidgetGyroscope_setOnAngularRateUpdateHandler(PhidgetGyroscopeHandle ch,
  PhidgetGyroscope_OnAngularRateUpdateCallback fptr, void *ctx) {
	...
	MEMORY_BARRIER();
	if (fptr == NULL) {
		ch->AngularRateUpdate = NULL;
		MEMORY_BARRIER();
		ch->AngularRateUpdateCtx = NULL;
	} else {
