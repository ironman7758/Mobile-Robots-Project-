...
struct _PhidgetHumiditySensor {
	...
	volatile PhidgetHumiditySensor_OnHumidityChangeCallback HumidityChange;
	volatile void *HumidityChangeCtx;
};
...
_bridgeInput(PhidgetChannelHandle phid, BridgePacket *bp) {
	...
	case BP_HUMIDITYCHANGE:
		...
		FIRECH(ch, PhidgetHumiditySensor, HumidityChange, ch->humidity);
		break;
	default:
...
PhidgetHumiditySensor_setOnHumidityChangeHandler(PhidgetHumiditySensorHandle ch,
  PhidgetHumiditySensor_OnHumidityChangeCallback fptr, void *ctx) {
	...
	MEMORY_BARRIER();
	if (fptr == NULL) {
		ch->HumidityChange = NULL;
		MEMORY_BARRIER();
		ch->HumidityChangeCtx = NULL;
	} else {
