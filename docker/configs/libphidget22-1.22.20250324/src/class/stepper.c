...
PhidgetStepper_bridgeInput(PhidgetChannelHandle phid, BridgePacket *bp) {
	...
	case BP_POSITIONCHANGE:
		...
		FIRECH(ch, PhidgetStepper, PositionChange, ((ch->position + ch->positionOffset) * ch->rescaleFactor));
		res = EPHIDGET_OK;
		break;
	...
	case BP_VELOCITYCHANGE:
		...
		if (ch->velocity != 0)
			ch->isMoving = PTRUE;
		FIRECH(ch, PhidgetStepper, VelocityChange, (ch->velocity * ch->rescaleFactor));
		res = EPHIDGET_OK;
		break;
