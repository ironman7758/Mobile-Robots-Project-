...
struct _PhidgetCurrentInput {
	...
	volatile PhidgetCurrentInput_OnCurrentChangeCallback CurrentChange;
	volatile void *CurrentChangeCtx;
};
...
_bridgeInput(PhidgetChannelHandle phid, BridgePacket *bp) {
	...
	case BP_CURRENTCHANGE:
		...
		FIRECH(ch, PhidgetCurrentInput, CurrentChange, ch->current);
		break;
	default:
...
_initAfterOpen(PhidgetChannelHandle phid) {
	...
	case PHIDCHUID_DCC1020_CURRENTINPUT_100:
		...
		ch->maxDataRate = 100;
		ch->maxCurrent = 120;
		ch->maxCurrentChangeTrigger = 240;
		ch->minCurrent = -120;
		ch->minCurrentChangeTrigger = 0;
		ch->current = PUNK_DBL;
		ch->currentChangeTrigger = 0;
...
PhidgetCurrentInput_setOnCurrentChangeHandler(PhidgetCurrentInputHandle ch,
  PhidgetCurrentInput_OnCurrentChangeCallback fptr, void *ctx) {
	...
	MEMORY_BARRIER();
	if (fptr == NULL) {
		ch->CurrentChange = NULL;
		MEMORY_BARRIER();
		ch->CurrentChangeCtx = NULL;
	} else {
