...
handleFrequencyData(PhidgetFrequencyCounterHandle phid, double ticks, int counts, double ticksAtLastCount) {
	...
	if (counts == 0) {
		...
		if (phid->totalTicksSinceLastCount > cutoffTime) {
			phid->frequency = 0;
			FIRECH(phid, PhidgetFrequencyCounter, FrequencyChange, 0);
		}
	...
	if (phid->totalTicksSinceLastCount == PUNK_DBL) {
		...
		/* Initial count event */
		FIRECH(phid, PhidgetFrequencyCounter, CountChange, counts, ticksAtLastCount);
		return;
	}
	...
	if (frequency < phid->frequencyCutoff) {
		...
		FIRECH(phid, PhidgetFrequencyCounter, FrequencyChange, phid->frequency);
	}

	FIRECH(phid, PhidgetFrequencyCounter, CountChange, counts, (double)countTimeSpan);
}
