...
struct _PhidgetMagnetometer {
	...
	double timestamp;
	volatile PhidgetMagnetometer_OnMagneticFieldChangeCallback MagneticFieldChange;
	volatile void *MagneticFieldChangeCtx;
};
...
_bridgeInput(PhidgetChannelHandle phid, BridgePacket *bp) {
	...
	case BP_FIELDSTRENGTHCHANGE:
		...
		FIRECH(ch, PhidgetMagnetometer, MagneticFieldChange, ch->magneticField, ch->timestamp);
		break;
	default:
...
PhidgetMagnetometer_setOnMagneticFieldChangeHandler(PhidgetMagnetometerHandle ch,
  PhidgetMagnetometer_OnMagneticFieldChangeCallback fptr, void *ctx) {
	...
	MEMORY_BARRIER();
	if (fptr == NULL) {
		ch->MagneticFieldChange = NULL;
		MEMORY_BARRIER();
		ch->MagneticFieldChangeCtx = NULL;
	} else {
