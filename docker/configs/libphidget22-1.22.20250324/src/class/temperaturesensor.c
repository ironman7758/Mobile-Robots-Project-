...
extern const int rtd_useful_range[5][2];
...
PhidgetTemperatureSensor_bridgeInput(PhidgetChannelHandle phid, BridgePacket *bp) {
	...
	case BP_SETRTDTYPE:
		...
		if (res == EPHIDGET_OK) {
			...
			ch->minTemperature = rtd_useful_range[(PhidgetTemperatureSensor_RTDType)getBridgePacketInt32(bp, 0)][0];
			ch->maxTemperature = rtd_useful_range[(PhidgetTemperatureSensor_RTDType)getBridgePacketInt32(bp, 0)][1];
#endif
			PhidgetRunUnlock(ch);
#if PHIDUID_TMP1202_0_SUPPORTED
