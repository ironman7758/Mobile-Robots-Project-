...
Phidget_strerrordetail(PhidgetReturnCode code) {
	...
	case EPHIDGET_RESOLV:
		return ("");
	case EPHIDGET_UNKNOWNVAL:
		return ("The value is unknown. This can happen right after attach, when the value has not yet been received from the Phidget. This can also happen if a device has not yet been configured / enabled. Some properties can only be read back after being set.");
	case EPHIDGET_NOTATTACHED:
	...
	case EPHIDGET_HALLSENSOR:
		return ("The hall sensor on your Brushless DC Motor Controller is Improperly Connected");
	case EPHIDGET_BADCURRENT:
		return ("Current sensor offset outside acceptable bounds. Move the sensor aways from magnetic fields and try again.");
	case EPHIDGET_BADCONNECTION:
		return ("One or more required connections on the device has been deemed faulty. Check your connections and try again.");
	case EPHIDGET_NACK:
		return ("An external device has responded with a NACK response. Evaluate whether this is expected and try again.");
	default:
		return ("Invalid return code");
	}
}
