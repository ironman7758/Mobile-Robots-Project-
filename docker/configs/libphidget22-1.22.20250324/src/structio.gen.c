...
PhidgetReturnCode
readRGBW(BridgePacket *bp, PhidgetLEDArray_RGBWHandle dst) {

	dst->r = getBridgePacketUInt8ByName(bp, "RGBW.r");
	dst->g = getBridgePacketUInt8ByName(bp, "RGBW.g");
	dst->b = getBridgePacketUInt8ByName(bp, "RGBW.b");
	dst->w = getBridgePacketUInt8ByName(bp, "RGBW.w");

	return (EPHIDGET_OK);
}

PhidgetReturnCode
writeRGBW(const PhidgetLEDArray_RGBWHandle src, BridgePacket *bp) {
	PhidgetReturnCode res;

	res = addBridgePacketUInt8(bp, src->r, "RGBW.r");
	if (res != EPHIDGET_OK)
		return (res);

	res = addBridgePacketUInt8(bp, src->g, "RGBW.g");
	if (res != EPHIDGET_OK)
		return (res);

	res = addBridgePacketUInt8(bp, src->b, "RGBW.b");
	if (res != EPHIDGET_OK)
		return (res);

	res = addBridgePacketUInt8(bp, src->w, "RGBW.w");
	if (res != EPHIDGET_OK)
		return (res);
...
PhidgetReturnCode
readAnimationDescription(BridgePacket *bp, PhidgetLEDArray_AnimationDescriptionHandle dst) {

	dst->startAddress = getBridgePacketUInt16ByName(bp, "AnimationDescription.startAddress");
	dst->span = getBridgePacketUInt16ByName(bp, "AnimationDescription.span");
	dst->time = getBridgePacketUInt16ByName(bp, "AnimationDescription.time");
	dst->animationType = getBridgePacketInt32ByName(bp, "AnimationDescription.animationType");

	return (EPHIDGET_OK);
}

PhidgetReturnCode
writeAnimationDescription(const PhidgetLEDArray_AnimationDescriptionHandle src, BridgePacket *bp) {
	PhidgetReturnCode res;

	res = addBridgePacketUInt16(bp, src->startAddress, "AnimationDescription.startAddress");
	if (res != EPHIDGET_OK)
		return (res);

	res = addBridgePacketUInt16(bp, src->span, "AnimationDescription.span");
	if (res != EPHIDGET_OK)
		return (res);

	res = addBridgePacketUInt16(bp, src->time, "AnimationDescription.time");
	if (res != EPHIDGET_OK)
		return (res);

	res = addBridgePacketInt32(bp, src->animationType, "AnimationDescription.animationType");
	if (res != EPHIDGET_OK)
		return (res);
