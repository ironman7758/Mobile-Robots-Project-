...
static volatile PhidgetNet_OnServerRemovedCallback serverRemoved;
static volatile void *serverRemovedCtx;
static volatile PhidgetNet_OnServerAddedCallback serverAdded;
static volatile void *serverAddedCtx;
...
wwwServerListener(ZeroconfListenerHandle handle, void *ctx, int added, int interface, Zeroconf_Protocol proto,
	...
	if (!added) {
		PhidgetNet_OnServerRemovedCallback handler = serverRemoved;
		void *rctx = (void *)serverRemovedCtx;
		if (handler) {
			server.name = name;
	...
	PhidgetNet_OnServerAddedCallback handler = serverAdded;
	void *actx = (void *)serverAddedCtx;
	if (handler) {
...
deviceServerListener(ZeroconfListenerHandle hdl, void *ctx, int added, int interface, Zeroconf_Protocol proto,
	...
	netloginfo("Discovered server '%s' [%s] (interface %d)", name, host, interface);

	res = PhidgetNet_discoveredServer(hdl, 0, interface, proto, PHIDGETSERVER_DEVICEREMOTE, name, host, type, domain, -1);
	if (res != 0)
	...
	case PHIDGETCONN_LOCAL: /* Server */
		...
		if (nc->keepalive_dl != 0 && nc->keepalive_dl < tm) {
			...
			if (nc->keepalive_dl == 0) {
				...
				netlogverbose("%" PRIphid ": keepalive check passed - keepalive: [%" PRId64 " usec] remaining: [%" PRId64 " usec]", nc,
...
runKeepAlive(void *arg) {
	...
	while (nc->errcondition == 0) {
		...
		if (nc->keepalive_dl == 0) {
			...
			netlogverbose("%" PRIphid ": keepalive check sleeping(2) for %" PRId64 " us", nc, (nc->keepalive_dl - tm));
...
closeIPhidgetServer(IPhidgetServerHandle *_server) {
	...
	PhidgetNet_OnServerRemovedCallback handler = serverRemoved;
	void *ctx = (void *)serverRemovedCtx;
	if (handler)
		handler(ctx, &server->info);
...
PhidgetNet_enableServerDiscovery(PhidgetServerType srvt) {
	...
	case PHIDGETSERVER_DEVICELISTENER:
	case PHIDGETSERVER_DEVICE:
		srvt = PHIDGETSERVER_DEVICEREMOTE;
		break;
	case PHIDGETSERVER_WWWLISTENER:
	case PHIDGETSERVER_WWW:
		srvt = PHIDGETSERVER_WWWREMOTE;
		break;
	}
...
static PhidgetReturnCode
setPropertyv(const char *key, const char *fmt, va_list va) {
	PhidgetReturnCode res;
	char val[1024];
	int len;
...
PhidgetNet_setProperty(const char *key, const char *fmt, ...) {
	...
	va_start(va, fmt);
	res = setPropertyv(key, fmt, va);
	va_end(va);

	// lastError set by setPropertyv
	return (res);
}
