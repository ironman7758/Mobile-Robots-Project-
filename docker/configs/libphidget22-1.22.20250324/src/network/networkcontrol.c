...
runNetworkControlEntry(void *arg) {
	...
	while (nce->flags & NCE_ENABLED) {
		...
		nclogdebug("network control entry '%s' not connected (failures=%d)", nce->name, nce->failedconn);
		...
		if ((nce->flags & NCE_MDNS) && nce->handle) {
			...
			addrlist = NULL;
			res = Zeroconf_lookup(nce->handle, nce->interface, nce->zcproto, nce->name, nce->host, nce->type,
								  nce->domain, ZCP_IPv4, &addrlist, &port, &txt);
			if (res != EPHIDGET_OK) {
				nce->failedconn++;
				nce->flags |= NCE_LOOKUPF;
				nclogerr("Zeroconf_lookup() failed for host '%s' [%s] on interface %d: " PRC_FMT, nce->name, nce->host, nce->interface, PRC_ARGS(res));
				goto done;
			}
			...
			if (i == PHIDGET_MDNS_TXTVER) {
				...
				if (i != PHIDGET_NET_PROTOCOL_MAJOR) {
					nclogwarn("server '%s' (%s:%d) broadcasting protocol version %d: ignoring server", nce->name, taddr, nce->port, i);
					nce->flags |= NCE_BADVERSION;
					goto done;
				}
			} else {
				nclogwarn("server '%s' (%s:%d) broadcasting txtvers version %d: ignoring server", nce->name, taddr, nce->port, i);
				nce->flags |= NCE_BADVERSION;
				goto done;
			}
			...
			ncloginfo("network control entry '%s' connected to %s (%s:%d)", nce->name, nce->host, taddr, nce->port);
		} else
#endif
		{ /* ! NCE_MDNS */
	...
	if ((nce->flags & NCE_ENABLED) == 0)
		...
		nclogwarn("network control entry '%s' failing (0x%x)", nce->name, nce->flags);
...
_addPhidgetServer(PhidgetServerType srvt, int flags, int lock, mos_af_t proto, const char *name,
	...
	if (nce->flags & NCE_ENABLED) {
		/*
		 * Lock not required here, since we are not in the list yet and can only be seen by this thread.
		 */
...
PhidgetReturnCode
PhidgetNet_discoveredServer(ZeroconfListenerHandle handle, int flags, int interface, Zeroconf_Protocol proto, PhidgetServerType srvt,
	...
	if (nce != NULL) {
		...
		/*
		 * This entry was created by calling PhidgetNet_setServerPassword(), we convert it to an mDNS entry
		 */
		if (nce->flags & NCE_VIRTUAL) {
			ncloginfo("Adding discovered server: '%s'", name);
		 ...
		 * This could also be a server being discovered on multiple interfaces
		 */
		if (nce->flags & NCE_MDNS) {
			...
			/* We have an active thread, leave it alone */
			if ((nce->flags & (NCE_ENABLED | NCE_RUNNING)) == (NCE_ENABLED | NCE_RUNNING)) {
				// This triggers the nce thread to re-try the connection immidiately if it's in the waiting state
				mos_cond_broadcast(&nce->cond);
				mos_tlock_unlock(nce->lock);
				nclogdebug("network control entry for '%s' rediscovered", name);
				return (EPHIDGET_OK);
			}

			/* Update the info in the nce and enable the entry */
			if (nce->name) {
		...
		/*
		 * We already added a server with this name using PhidgetNet_addServer(), so we just ignore the one discovered via mDNS
		 */
		mos_tlock_unlock(nce->lock);
		nclogwarn("not connecting to mDNS server '%s' because a server with the same name was added using PhidgetNet_addServer()", name);
	...
	if (res != EPHIDGET_OK) {
		nclogerr("failed to add MDNS discovered server '%s'", name);
		return (res);
	}
...
API_PRETURN
PhidgetNet_getServerAddressList(const char *hostname, int addressFamily, const char **addressList, uint32_t *count) {
...
API_PRETURN
PhidgetNet_freeServerAddressList(const char **addressList, uint32_t count) {
	uint32_t i;

	for (i = 0; i < count; i++)
		if (addressList[i] != NULL) {
			mos_free((char *)(uintptr_t)addressList[i], MOSM_FSTR);
			addressList[i] = NULL;
		}
